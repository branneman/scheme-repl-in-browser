...
function indent(code, level, offset) {
  ...
  if (last_sexpr) {
    ...
    if (last_sexpr.length === 1) {
      ...
      last_sexpr.length > 3 &&
      last_sexpr[1].line === last_sexpr[3].line
    ) {
...
jQuery(document).ready(function ($, undefined) {
  ...
  BiwaScheme.Port.current_input = new BiwaScheme.Port.CustomInput(function (
    callback
  ) {
  ...
  if (window.Prism) {
    ...
    Prism.languages.scheme.keyword.pattern =
      /(\()(?:define(?:-syntax|-library|-values)?|(?:case-)?lambda|let(?:\*|rec)?(?:-values)?|else|if|cond|begin|delay(?:-force)?|parameterize|define-macro|guard|set!|(?:quasi-)?quote|syntax-rules)(?=[()\s])/
  }
  var position
  var timer
